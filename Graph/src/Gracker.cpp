...
int Gracker::initModel(cv::Mat& frame, vector<cv::Point2f>& initPoints)
{
	...
	m_nPointGeoTol = (int)sqrt(rect.width * rect.width + rect.height * rect.height) / 10;
	m_nPointGeoTol = max(20, min(40, m_nPointGeoTol));
...
//	m_initGraph.setGrids(m_nModelGrids);
	...
	Warpping m_curWarpping_inv = m_curWarpping.inv();
	m_initGraph.transferNodeLoc(m_curWarpping_inv);
...
Warpping Gracker::processFrame(cv::Mat& frame)
{
	...
	if (!isWarppingValid(predWarp)) {
		bool bRet = m_detector.detect(frame, predWarp);
		if (bRet) {
	...
	while (1) {
		...
		cv::Mat warped; 
		cv::warpPerspective(frame, warped, H_, cv::Size(width + 2 * xshift, height + 2 * yshift));
		m_lWarpTime = m_lWarpTime + clock() - t0;
